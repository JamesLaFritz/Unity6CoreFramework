...
// Description: Stateless, deterministic noise-based RNG using 32-bit integer indexing inspired by SMU Guildhall's Squirrel Eiserloh's
...
{
    /// <summary>
    /// Stateless random number generator based on SquirrelNoise3.
    /// Supports deterministic access and reproducibility.
    /// </summary>
    public struct SquirrelNoise32Rng : IRandomFunction<int, uint>
    {
        /// <summary>
        /// Represents a random number generator utilizing Squirrel Noise for deterministic pseudo-random output.
        /// Supports generation of random floats, integers, colors, and vectors in various ranges and dimensions.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SquirrelNoise32Rng(uint seed, NoiseType type = NoiseType.MangledBitsBalancedMix, int position = 0)
        {
            Seed = seed;
            Type = type;
            Position = position;
        ...
        #region IRandomFunction<int, uint>
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => SquirrelNoise32Bit.Get1DNoise01(Position++, Seed);
        }
        ...
        public float3 InsideUnitSphere
        {
            ...
            get
            {
                var position = (int)SquirrelNoise32Bit.Get1DNoise(Position, Seed);
                var radius = SquirrelNoise32Bit.Get3DNoise01(position, (int)SquirrelNoise32Bit.Get2DNoise(position, Position, Seed), Position, Seed);
        ...
        public float2 InsideUnitCircle
        {
            ...
            get => NoiseRngUtils.InsideUnitCircle(
                SquirrelNoise32Bit.Get2DNoise01((int)SquirrelNoise32Bit.Get1DNoise(Position, Seed), Position, Seed), Value);
        }
        ...
        public float3 OnUnitSphere
        {
            ...
            get
            {
                ...
                var positionZ = (int)SquirrelNoise32Bit.Get1DNoise(position, Seed);
                var positionTheta = (int)SquirrelNoise32Bit.Get2DNoise(position, positionZ, Seed);
                
                var z = SquirrelNoise32Bit.Get1DNoise01(positionZ, Seed) * 2f - 1f;
                var theta = SquirrelNoise32Bit.Get2DNoise01(positionZ, positionTheta, Seed);
        ...
        public quaternion Rotation
        {
            ...
            get
            {
                ...
                return NoiseRngUtils.Rotation(axis,
                    SquirrelNoise32Bit.Get4DNoise01(position, (int)axis.x, (int)axis.y, (int)axis.z, Seed));
            }
        }
        ...
        public quaternion RotationUniform
        {
            ...
            {
                var position = Position;
                var positionX = (int)SquirrelNoise32Bit.Get1DNoise(position, Seed);

                return NoiseRngUtils.RotationUniform(Value, SquirrelNoise32Bit.Get2DNoise01(position, positionX, Seed),
                    SquirrelNoise32Bit.Get3DNoise01(position, positionX, (int)SquirrelNoise32Bit.Get2DNoise(position, positionX, Seed), Seed));
            }
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => SquirrelNoise32Bit.GetUInt128(Position++, Seed);
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => SquirrelNoise32Bit.GetUInt64(Position++, Seed);
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => SquirrelNoise32Bit.Get1DNoise(Position++, Seed);
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => NoiseRngUtils.Uint16(Uint32);
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => new (SquirrelNoise32Bit.Get1DNoiseNeg1To1(Position, Seed),
                SquirrelNoise32Bit.Get2DNoiseNeg1To1(Position, (int)SquirrelNoise32Bit.Get1DNoise(Position++, Seed), Seed));
        }
            ...
            float maxY = float.MaxValue)
        {
            ...
            return new float2(NoiseRngUtils.Range(minX, maxX, Value),
                NoiseRngUtils.Range(minY, maxY,
                    SquirrelNoise32Bit.Get2DNoise01(positionX, (int)SquirrelNoise32Bit.Get2DNoise(position, positionX, Seed), Seed)));
        }
        ...
        public float3 Direction3D
        {
            ...
            {
                var positionX = (int)SquirrelNoise32Bit.Get1DNoise(Position, Seed);
                var positionY = (int)SquirrelNoise32Bit.Get2DNoise(Position, positionX, Seed);
                return new float3(SquirrelNoise32Bit.Get1DNoiseNeg1To1(Position, Seed),
                    SquirrelNoise32Bit.Get2DNoiseNeg1To1(Position, positionX, Seed),
                    SquirrelNoise32Bit.Get3DNoiseNeg1To1(Position, positionX,
                        (int)SquirrelNoise32Bit.Get3DNoise(Position, positionX, positionY, Seed)));
            }
        }
            ...
            float maxY = float.MaxValue, float minZ = float.MinValue, float maxZ = float.MaxValue)
        {
            ...
            return new float3(NoiseRngUtils.Range(minX, maxX, Value),
                NoiseRngUtils.Range(minY, maxY, SquirrelNoise32Bit.Get2DNoise01(position, positionX, Seed)),
                NoiseRngUtils.Range(minZ, maxZ, SquirrelNoise32Bit.Get3DNoise01(position, positionX, positionY, Seed)));
        }
        ...
        public float4 Direction4D
        {
            get
            {
                var positionX = (int)SquirrelNoise32Bit.Get1DNoise(Position, Seed);
                var positionY = (int)SquirrelNoise32Bit.Get2DNoise(Position, positionX, Seed);
                var positionZ = (int)SquirrelNoise32Bit.Get3DNoise(Position, positionX, positionY, Seed);
                return new float4(SquirrelNoise32Bit.Get1DNoiseNeg1To1(Position, Seed),
                    SquirrelNoise32Bit.Get2DNoiseNeg1To1(Position, positionX, Seed),
                    SquirrelNoise32Bit.Get3DNoiseNeg1To1(Position, positionX, positionY, Seed),
                    SquirrelNoise32Bit.Get4DNoiseNeg1To1(positionX, positionY, positionZ,
                        (int)SquirrelNoise32Bit.Get4DNoise(Position++, positionX, positionY, positionZ, Seed), Seed));
            }
        }
            ...
            float minW = float.MinValue, float maxW = float.MaxValue)
        {
            ...
            return new float4(NoiseRngUtils.Range(minX, maxX, Value),
                ...
                NoiseRngUtils.Range(minW, maxW,
                    SquirrelNoise32Bit.Get4DNoise01(position, positionX, positionY, positionZ, Seed)));
        }
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public float Perlin(float x, float y) => SquirrelNoise32Bit.Perlin(x, y, Seed, Type);
        ...
        /// </returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public float Perlin(float x, float y, float z) => SquirrelNoise32Bit.Perlin(x, y, z, Seed, Type);
